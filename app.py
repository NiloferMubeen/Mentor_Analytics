...
def build_context(df):
    ...
    daily_trend = df.dropna(subset=['Date']).groupby('Date').size()
    date_labels = list(daily_trend.index.astype(str))
    ...
    unique_dates  = sorted(df['Date'].dropna().astype(str).unique())
        ...
        for tab_name, data_start_row in tab_config.items():
            try:
                ...
                if n_cols <= 26:
                    ...
                    first = chr(ord('A') + (n_cols - 1) // 26 - 1)
                    second = chr(ord('A') + (n_cols - 1) % 26)
                    last_col_letter = first + second
...
def run_tracker_alerts():
    ...
    for _, row in deadline_rows.iterrows():
        ...
        body = (
            ...
            f"  ✅ Plan for the next project "
            f"(Project {proj_no}) for Batch {batch}\n\n"
            f"  Batch         : {batch}\n"
